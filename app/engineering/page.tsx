...
import {
  engineeringCoursesBySem as defaultEngCourses,
  engineeringFacultyByCourseCode as defaultFacultyMap,
  STORE_KEYS,
} from "../../data/config"
import { CourseExitSurveyDBMSForm } from "../../components/forms/course-exit-survey-dbms"
...
function getCourses(sem: number): Course[] {
  const local = localStorage.getItem(STORE_KEYS.engCourses)
  if (local) {
    try {
  ...
  return defaultEngCourses[sem] || []
}

function getFacultyMap(): Record<string, string> {
  const local = localStorage.getItem(STORE_KEYS.engFaculty)
  if (local) {
    try {
      return JSON.parse(local)
    } catch {}
  }
  return defaultFacultyMap
}
...
export default function EngineeringDashboard() {
  ...
  const facultyMap = useMemo(() => getFacultyMap(), [])
  const facultyName = selected?.code ? facultyMap[selected.code] : undefined
    ...
    <main className="min-h-dvh bg-background text-foreground">
      ...
      <section className="w-full bg-primary/10 border-b border-primary/20">
        <div className="mx-auto max-w-5xl px-6 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-xl md:text-2xl font-semibold">Engineering Dashboard</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            {facultyName && (
              <span className="hidden md:inline-flex items-center rounded-full bg-accent px-3 py-1 text-xs font-medium text-accent-foreground">
                {facultyName}
              </span>
            )}
          </div>
            ...
            {selected && (
              <Tabs defaultValue="course" className="w-full">
                <TabsList>
                  ...
                  {sem === 8 && <TabsTrigger value="exit">Course Exit Survey</TabsTrigger>}
                </TabsList>
